...
from my_functions import get_class_distribution, make_cm, dummy_data, make_pretty_cm, convert_batch_list, visualize_imgs
from my_functions import train_cnn, infer_cnn, predict_cnn, val_cnn, extract_features, custom_classifier, FocalLoss, epoch_test_cnn
from pytorchtools import EarlyStopping_v2
...
if __name__ == "__main__": # For pycharm 

    artifact = "blood" #["damage", "blood", "airbubble", "blur", "fold"]
    dummy = False # to choose CIFAR10 for dummy testing
    architectures = [ "VGG16"] #[ "VGG16", "GoogleNet", "MobileNet", "ResNet", "DenseNet"] 
    ...
    class_to_predict = 'artifact_free'
    ...
    if artifact == "damage":
        if color_normalization == "Macenko":
            path_to_dataset = "/path_to/artifact_dataset/damage_macenko"
        elif color_normalization == "Vahadane":
            path_to_dataset = "/path_to/artifact_dataset/damage_vahadane"
        else:
            path_to_dataset = "/path_to/artifact_dataset/damage"

    elif artifact == "blood":
        if color_normalization == "Macenko":
            path_to_dataset = "/path_to/artifact_dataset/blood_macenko"
        elif color_normalization == "Vahadane":
            path_to_dataset = "/path_to/artifact_dataset/blood_vahadane"
        else:
    ...
    if data_augmentation:
        ...
        if grayscale:
            print("Using grayscale on train compose...")
            train_compose = transforms.Compose([
                transforms.Resize((224, 224)),
    ...
    if grayscale:
        print("Using grayscale on validation compose...")
        val_compose = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.Grayscale(num_output_channels=3),
            transforms.ToTensor(),
        ...
        print("Using grayscale for test compose... ")
        test_compose = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.Grayscale(num_output_channels=3),
            transforms.ToTensor(),
    ...
    t = time.time()
    if dummy:
        train_compose = transforms.Compose([transforms.ToTensor(),
                                            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                 std=[0.229, 0.224, 0.225])])
        val_compose = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
        train_loader, val_loader = dummy_data(BATCH_SIZE, train_compose, val_compose)
        num_classes = len(train_loader.dataset.classes)
        classes_list = val_loader.dataset.classes
        print(f"Shape of Input data {val_loader.dataset.data.shape}")
    else:
        ...
        if grayscale:
            visualize_imgs(train_loader, artifact)
            ...
            for sch in lr_scheduler:
                for lr in learning_rate:
                    ...
                    for i in range(iterations):
                        ...
                        param_dict = {"CUDA": cuda_device,
                                      "BATCH_SIZE": BATCH_SIZE,
                                      "EPOCHS": n_epochs,
                                      "PATIENCE": patience,
                                      "Learning Rate": lr,
                                      "PREDICTION": class_to_predict,
                                      "Optimizer": op,
                                      "LR Scheduler": sch,
                                      "Artifact": artifact,
                                      "Model": architecture,
                        ...
                        # y_pred, y_true, probs, feature = predict_cnn(val_loader, model)
                        ...
                        if not dummy:
                            ...
                            # data = {"files": file_names, "ground_truth": convert_batch_list(y_true), "prediction": convert_batch_list(y_pred), "probabilities": convert_batch_list(probs)}
                            ...
                            if extract_feature:
                                if architecture == "DenseNet" or architecture =="DenseNetConfig":
                                    ...
                                    # y_pred, y_true, probs, feature = predict_cnn(train_loader, model)
                                    # file_names = [im[0].split("/")[-1] for im in train_loader.dataset.imgs]
                                    # data = {"files": file_names, "ground_truth": y_true, "prediction": y_pred, "probabilities": probs}
                                    # dframe = pd.DataFrame(data)
                                    #
                                    # with pd.ExcelWriter(f"{path}/predictions_for_training_{str(artifact)}.xlsx") as wr:
                                    #     dframe.to_excel(wr, index=False)
                                    # with open(f"{path}/{str(artifact)}_training_features_fc.npy", 'wb') as f:
                                    #     np.save(f, feature)
                                    # feature_x = extract_features(model, train_loader)
                                    # with open(f"{path}/{str(artifact)}_training_features_original.npy", 'wb') as f:
                                    #     np.save(f, feature_x)

                            with open(f"{path}/Classification Report (val).txt", "a+") as f:
                            ...
                            if test:
                                ...
                                # y_pred, y_true, probs, feature = predict_cnn(test_loader, model)
